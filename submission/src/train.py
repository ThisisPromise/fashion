"""Training entrypoint. Command used for the submitted checkpoint:

    python -m src.train --config configs/train_config.json
"""
...
from src.dataset import GarmentSegDataset, MASK_DIR, SPLIT_MANIFEST_PATH
from src.model import TinyGarmentSegModel, count_frozen_parameters, count_trainable_parameters


def compute_class_weights(num_classes):
    """Sqrt-dampened inverse pixel frequency, from the train split only."""
    train_ids = []
    ...
    weights = weights / weights.mean()  # keep loss magnitude comparable to unweighted CE
...
def compute_confusion(preds, targets, num_classes):
    """Accumulate a (num_classes, num_classes) confusion matrix, rows=true, cols=pred."""
...
def main():
    ...
    train_ds = GarmentSegDataset("train", image_size=cfg["image_size"])
    val_ds = GarmentSegDataset("val", image_size=cfg["image_size"])
    ...
    # Unfrozen backbone layers (if any) get a smaller LR than the decoder --
    # standard fine-tuning practice, since they start from useful pretrained
    # weights that a full-strength LR could distort too quickly.
    ...
    class_weights = compute_class_weights(cfg["num_classes"])
    ...
    # Final metrics from the best (lowest val loss) checkpoint, not the last epoch.
